...
while {true} do
{
    if (!isNull player) then {
        private ["_currentVehicle", "_isNearFeed", "_countFuel"];
                ...
                _countFuel = _countFuel + (count ((position _currentVehicle) nearObjects ["land_nav_pier_M_fuel", _distance]));
        ...
        if (_vehicle != _currentVehicle) then {
            ...
            if (_currentVehicle != player && _isNearFeed && !(_currentVehicle isKindof "Bicycle")) then {  //change "Bicycle" to "Land" to allow only air vehicles to aut-refuel
                ...
                _vehicle_refuel_id = _vehicle addAction ["Refuel and Repair", "custom\refuel\repair.sqf", [_amount,_amount2], -1, false, true, "", "vehicle _this == _target && local _target"];
            };
        };
