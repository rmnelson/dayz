...
// ------------------------------------------------------------------------Krixes Self Bloodbag Start------------------------------------------------------------------------
	...
	if((speed player <= 1) && hasBagItem && _canDo) then {
		if (s_player_selfBloodbag < 0) then {
			s_player_selfBloodbag = player addaction[("<t color=""#c70000"">" + ("Self Bloodbag") +"</t>"),"custom\bloodbag\player_selfbloodbag.sqf","",5,false,true,"", ""];
		};
	} else {
	...
	*/
	if (_isMan and !_isAlive and !_isZombie) then {
		if (s_player_studybody < 0) then {
			s_player_studybody = player addAction [localize "str_action_studybody", "custom\studybody\study_body.sqf",_cursorTarget, 0, false, true, "",""];
		};
	} else {
...
if((speed player <= 1) && hasSecondary && _canDo) then {
    if (s_player_suicide < 0) then {
        s_player_suicide = player addaction[("<t color=""#ff0000"">" + ("Commit Suicide") +"</t>"),"custom\suicide\suicide.sqf",_handGun,0,false,true,"", ""];
    };
} else {
